...
namespace ACT.Core.Extensions
{
   public static class String_Extensions
   {
      ...
      /// <returns></returns>
      public static string CleanComments(this string x, Dictionary<string, string> CommentTags, bool StartsLine = true, Dictionary<string, string> SpanningLinesCommentStartEndTags = null)
      {
         string _TmpReturn = "";
         var Intermediate_Data = x;

         if (SpanningLinesCommentStartEndTags != null && SpanningLinesCommentStartEndTags.Keys.Count() > 0 && SpanningLinesCommentStartEndTags.First().Value.NullOrEmpty() == false && SpanningLinesCommentStartEndTags.First().Key.NullOrEmpty() == false)
         {
            // Remove All MultiLine Comment Indicators
            foreach (var tag in CommentTags.Keys)
            {
               Regex _CommentStripper = new Regex("\\/\\*[\\s\\S]*?\\*\\/", RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Compiled);
               var _MatchData = _CommentStripper.Matches(x);
               int _IndexOffset = 0;
               foreach (Match _Match in _MatchData)
               {
                  Intermediate_Data = Intermediate_Data.Remove(_Match.Index - _IndexOffset, _Match.Length);
                  _IndexOffset += _Match.Length;
               }
            }
         }

         foreach (string line in Intermediate_Data.ReplaceLineEndings().SplitString(Environment.NewLine, StringSplitOptions.None))
         {
            bool _add = true;
            foreach (var tag in CommentTags.Keys)
            {
               string TagLineData = x.Trim();
               if (TagLineData.StartsWith(tag)) { _add = false; break; }
            }
            if (_add) { _TmpReturn = _TmpReturn + line; }
         }
         return _TmpReturn;
      ...
      {
         if (!CaseSensitive) { x = x.ToLower(); }

         foreach (string option in Options)
         {
            if (!CaseSensitive)
            {
               if (x.EndsWith(option.ToLower())) { return true; }
            }
      ...
      public static bool ToBool(this string x, bool Default = false)
      {
         ...
         if (bool.TryParse(x, out _TmpReturn)) { return (bool)_TmpReturn; }
         else { return Default; }
      }
      ...
      public static DateTime? TryToDateTime(this string x)
      {
         ...
         if (DateTime.TryParse(x, out _TmpReturn)) { return (DateTime?)_TmpReturn; }
      ...
      {
         int _tmpReturn = x.ToInt(Common_Constants.TESTINTVALUE);
         if (_tmpReturn == Common_Constants.TESTINTVALUE) { return null; }
         else { return (int?)_tmpReturn; }
      }
      ...
      public static Guid? TryToGuid(this string x)
      {
         ...
         if (Guid.TryParse(x, out _tmpReturn)) { return (Guid?)_tmpReturn; }
      ...
      {
         decimal _tmpReturn = x.ToDecimal(Common_Constants.TESTDECIMALVALUE);
         if (_tmpReturn == Common_Constants.TESTDECIMALVALUE) { return null; }
         else { return (decimal?)_tmpReturn; }
      }

      /// <summary>   Converts A String To A double.  If Error Returns Null. </summary>
      /// <remarks>   Mark Alicz, 12/8/2016. </remarks>
      /// <param name="x">    . </param>
      /// <returns>   double or Null. </returns>
      public static double? TryToDouble(this string x)
      {
         double _tmpReturn = x.ToDouble(Common_Constants.TESTDOUBLEVALUE);
         if (_tmpReturn == Common_Constants.TESTDOUBLEVALUE) { return null; }
         else { return (double?)_tmpReturn; }
      }
      ...
      {
         double? _MoneyVal = x.TryToDouble();
         if (_MoneyVal == null) { return "$0.00"; }
         else
         {
      ...
      {
         int _TmpReturn = -1;
         if (int.TryParse(x, out _TmpReturn))
         {
            return _TmpReturn;
         }
         else
      ...
      {
         long _TmpReturn = -1;
         if (long.TryParse(x, out _TmpReturn))
         {
            return _TmpReturn;
         }
         else
      ...
      {
         decimal _TmpReturn = -1;
         if (decimal.TryParse(x, out _TmpReturn))
         {
            return _TmpReturn;
         }
         else
      ...
      {
         DateTime _TmpReturn = DateTime.MinValue;
         if (DateTime.TryParse(x, out _TmpReturn))
         {
            return _TmpReturn;
         }
         else
      ...
      public static int? EnsureInt(this string x, bool SetNull)
      {
         ...
         if (x.ToInt(-123451028) == -123451028)
         {
            if (SetNull) { return (int?)null; }
            return (int?)0;
         }
         else
         {
            return (int?)x.ToInt(-1);
         }
      }
